...
extern "C" int decode_ldpc_general_short(FP_Decoder* p_decoder, float* in, 
						unsigned char* uchar_out, int in_len_bit, int pad_len)
{
	...
	{
		cout << "codeword length not match, constant is " << CWD_LENGTH 
	...
	for(cwd_cnt = 0; cwd_cnt < 8; cwd_cnt++)
	{
		...
		counter = 0;
		//cout << " cwd_cnt " << cwd_cnt << ": ";
		// the last pad_len of the codewords are dummy bits
		if(0)
		{	
			for(i = 0; i < CWD_LENGTH - pad_len; i++)
			...
			{	
				...
				//cout << in[VarAddr] << " "; 
				int_in[counter] = int(-4*in[VarAddr] * (1 << FRAC_WIDTH) );
				counter ++;
			}
			// the last pad_len of the codewords are dummy bits
			for(i = CWD_LENGTH - pad_len; i < CWD_LENGTH; i++)
			{
				VarAddr = cwd_cnt + i*8;
				//cout << (in[VarAddr] > 0?1:0);
				// assign the largest channel value possible (dummy bits are 0)
				int_in[counter] = int(-20*(-0.5) * (1 << FRAC_WIDTH)); //0xffff
				counter ++;
			}
		}
		else
						...
						unsigned char* uchar_out, int in_len_bit)
{
	...
	for(cwd_cnt = 0; cwd_cnt < 8; cwd_cnt++)
	{
		...
		//cout << endl;
		//cout << "calling decoder_general\n";
		p_decoder -> setState(PCV);
		iter = p_decoder -> decode_general_fp(int_in);
		//if(iter > 1 && iter < MAX_ITER) cout << iter << endl;
		iter_count += iter;
		p_decoder -> loadInfoBit_intr(uchar_out, cwd_cnt);
	}
	//if(iter_count > 8 &&  iter_count < MAX_ITER )
	//	cout << "avg iter= " << iter_count/8 <<endl;

	//cout << "finished decode ldpc general\n";;
	return 0;
}
