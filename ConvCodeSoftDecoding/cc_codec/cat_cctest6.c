...
int main(int argc,char *argv[]){
  ...
  int incclen = 2000;
  const int nbytes = 16000;  // must be multiples of incclen
  ...
  // Encode
  cc6_encode(block, nbytes, incclen);

  codelength = cc6_codelength(nbytes, incclen);
  ccnbytes = cc6_nbytes(codelength, outcclen);
  
  //printf("outcclen is  %d \n",outcclen);
  //printf("codelength in byte is %d \n",codelength);
  //printf("nbytes is %d, calculated nbytes is %d \n",nbytes, ccnbytes);
  // Decode
  cc6_decode(block, codelength, outcclen);
  ...
  printf ("Input to Convolutional Encoder has length %d. Total number of frame is %d \n",incclen, nbytes/incclen);
  printf ("\n total err is %d \n", toterrcnt);
}
