...
int cc2_codelength(int nbytes, int incclen)
{
  int chunks;
...
int cc2_nbytes(int codelength, int outcclen)
{
  int chunks;
  int incclen;
  int nbytes;
...
void cc2_encode(unsigned char* block, int nbytes, int incclen)
{
	...
	outbytelen = cc2_codelength(nbytes, incclen);
	...
	for(n=0;n<chunks;n++){
		for(i=0;i<incclen+MEMLEN;i++){
			...
			bitind = 7 - ( n*outcclen+RATEINV*i+0)%8;
			symbols[byteind] = symbols[byteind] + (parity(sr & V27POLYA)<<bitind);
			byteind = ( n*outcclen+RATEINV*i+1)/8;
			//bitind = ( n*outcclen+RATEINV*i+1)%8;
			bitind = 7- ( n*outcclen+RATEINV*i+1)%8;
			symbols[byteind] = symbols[byteind] + (parity(sr & V27POLYB)<<bitind);
...
void cc2_decode(unsigned char* block, int codelength, int outcclen){
        /*
	...
	if((vp = create_viterbi27(incclen)) == NULL){
	    ...
	    /* Decode block */
	    update_viterbi27_blk(vp,insymbol+i*outcclen,incclen+MEMLEN);
	      
	    /* Do Viterbi chainback */
	    chainback_viterbi27(vp,data,incclen,0);
	   ...
	   // printf("\n decoded data is :");
		...
		data[j/8] = (data[j/8] << 1);
	    	//block[i*framebits + j] = ((data[j/8] & 0x80) >> 7);
	    	//data[j/8] = (data[j/8] << 1);
                //printf("%d ", decbit);
...
void cc2_softdecode(const unsigned char *insym, unsigned char *outsym, int codelength, int outcclen)
{
	...
	if((vp = create_viterbi27(incclen)) == NULL){
	    ...
	    // Decode block 
	    update_viterbi27_blk(vp,insym+i*outcclen,incclen+MEMLEN);
